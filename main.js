(function () {
    ...
    var DrawFunctions = (function() {
        ...
        function drawSelectedCellBorder() {
            ...
            borderDiv.style.top = `${pos.y + 1+110}px`;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // const formulaBarHeight = document.querySelector(".formula-bar").offsetHeight;
        // const offsetY = toolbarHeight + formulaBarHeight;
    ...
    const funcList = ["SUM", "AVERAGE", "MIN", "MAX","COUNT","UPPER"];
    ...
    function run_upper_func(paramString, outputCellName) {
        ...
        if (!cellNames || cellNames.length !== 2) return "#NaN"; // Ensure two cells are provided in the range.
        ...
        if (c1.col !== c2.col) {
            return "Error: Range must be in the same column.";
        }
    
        const column = c1.col; // The column to process.
    
        // Iterate over the rows in the range.
        for (let r = Math.min(c1.row, c2.row); r <= Math.max(c1.row, c2.row); r++) {
            const v = spreadsheetData[r][column]; // Access the cell content.
            if (typeof v === "string") {
                spreadsheetData[r][column] = v.toUpperCase(); // Convert to uppercase.
            }
        }
    ...
    function run_lower_func(paramString, outputCellName="B") {
        const cellRange = paramString;
        const cellNameRegex = /[A-Z]+\d+/g;
        const cellNames = cellRange.match(cellNameRegex);
        console.log(cellNames);
        if (cellNames.length < 1) return "#gOODED";
        ...
        for (let r = Math.min(c1.row, c2.row); r <= Math.max(c1.row, c2.row); r++) {
            for (let c = Math.min(c1.col, c2.col); c <= Math.max(c1.col, c2.col); c++) {
                const v = spreadsheetData[r][c];
                if (typeof v === "string") {
                    spreadsheetData[r][c] = v.toLowerCase();
                }
            }
        }
        return " ";
    ...
    function run_remove_duplicates_func(paramString, outputCellName) {
        ...
        for (let r = Math.min(c1.row, c2.row); r <= Math.max(c1.row, c2.row); r++) {
            const row = spreadsheetData[r]
                .slice(Math.min(c1.col, c2.col), Math.max(c1.col, c2.col) + 1)
                .join("|");
            if (uniqueRows.has(row)) {
                spreadsheetData.splice(r, 1); // Remove duplicate row
                r--; // Adjust row index after removal
            } else {
                uniqueRows.add(row);
            }
        }
    ...
    function run_find_and_replace_func(paramString, findText, replaceText, outputCellName) {
        ...
        for (let r = Math.min(c1.row, c2.row); r <= Math.max(c1.row, c2.row); r++) {
            for (let c = Math.min(c1.col, c2.col); c <= Math.max(c1.col, c2.col); c++) {
                const v = spreadsheetData[r][c];
                if (typeof v === "string") {
                    spreadsheetData[r][c] = v.replace(new RegExp(findText, "g"), replaceText);
                }
            }
        }
    ...
    // Adding to function map
    // funcMap["TRIM"] = run_trim_func;
    // funcMap["UPPER"] = run_upper_func;
    // funcMap["LOWER"] = run_lower_func;
    // funcMap["REMOVE_DUPLICATES"] = run_remove_duplicates_func;
    // funcMap["FIND_AND_REPLACE"] = run_find_and_replace_func;
    // const funcMap = {
    //     "TRIM": run_trim_func,
    //     "UPPER": run_upper_func,
    //     "LOWER": run_min_func,
    ...
    function displayInput() {
        ...
        textInput.style.top = `${pos.y + 1 + 110}px`;
